...
const obtener_pagos_x_mes = async ( req = request, res = response ) => {
    ...
    try {
        const pagos_del_mes = await prisma.$queryRaw`SELECT C.ID_CUOTA_SOCIO, CONCAT (A.NOMBRE, ' ', A.APELLIDO) AS NOMBRE_SOCIO, B.ID_SOCIO,
...
const obtener_cuotas_pendientes_x_socio = async ( req = request, res = response ) =>{
    ...
    try {
        const cuotas_pendientes = await prisma.$queryRaw`SELECT C.ID_CUOTA_SOCIO ,CONCAT (A.NOMBRE, ' ', A.APELLIDO) AS NOMBRE_SOCIO, 
                                                                B.ID_SOCIO, C.FECHA_VENCIMIENTO, D.NRO_FACTURA, D.MONTO_ABONADO, D.FECHA_PAGO
                                                            FROM PERSONA A JOIN SOCIO B ON A.ID_PERSONA = B.ID_PERSONA
                                                            JOIN CUOTAS_SOCIO C ON C.ID_SOCIO = B.ID_SOCIO
                                                            JOIN PAGOS_SOCIO D ON D.ID_CUOTA_SOCIO = C.ID_CUOTA_SOCIO
